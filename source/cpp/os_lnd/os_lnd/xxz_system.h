...
struct XXZSystem : BaseSystem
{
	...
	void operator()(const Eigen::VectorXcd& x, Eigen::VectorXcd& dxdt, const double t) override
	{
		...
		{
			double driving = ampl * std::cos(freq * t + phase);
			dxdt.noalias() = (model.lindbladian + driving * model.lindbladians_drv[4] + c0 * model.lindbladians_drv[0] + c1 * model.lindbladians_drv[1] + c2 * model.lindbladians_drv[2] + c3 * model.lindbladians_drv[3]) * x;
		}
	}
};
