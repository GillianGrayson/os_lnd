...
for p_id = 1:size(ps, 1)
    ...
    path = '/data/condmat/ivanchen/yusipov/os_lnd/regular/super_decoh/eigen_dense/method_simple';
	...
	shift_x = max(real(all_evals)) + 1e-16 - (min(real(all_evals)) - 1e-16);
	shift_y = max(imag(all_evals)) + 1e-16 - (min(imag(all_evals)) - 1e-16);
	
    pdf2d.x_bin_s = min(real(all_evals)) - 1e-16 - shift_x * 0.1;
    pdf2d.x_bin_f = max(real(all_evals)) + 1e-16 + shift_x * 0.1;
    pdf2d.y_bin_s = min(imag(all_evals)) - 1e-16 - shift_y * 0.1;
    pdf2d.y_bin_f = max(imag(all_evals)) + 1e-16 + shift_y * 0.1;
