...
rss_peak = zeros(size(nums_particles, 1), 1);
...
for n_id = 1 : size(nums_particles, 1)
    ...
    fn = sprintf('%s/%s/mem_info_%s.txt', data_path, local_path, suffix);
    data = importdata(fn);
    
    rss_peak(n_id) = data(2);
end

fig = figure;
hLine = plot(nums_particles, rss_peak, 'LineWidth', 2);
set(gca, 'FontSize', 20);
xlabel('$N$', 'Interpreter', 'latex');
set(gca, 'FontSize', 20);
ylabel('RSS peak, Mb', 'Interpreter', 'latex');
...
savefig(sprintf('%s/%s/memory_from_np_%s.fig', figures_path, task_path, suffix));

h=gcf;
set(h,'PaperOrientation','landscape');
set(h,'PaperUnits','normalized');
set(h,'PaperPosition', [0 0 1 1]);
print(gcf, '-dpdf', sprintf('%s/%s/memory_from_np_%s.pdf', figures_path, task_path, suffix));